...
# def get_symmetric_invertible_matrix_1(n):
...
# def get_symmetric_invertible_matrix_2(n):
#     mat = np.random.randint(low=-100, high=100, size=(n, n))
#     mat = mat + mat.T
...
def get_symmetric_invertible_matrix_3(n):
    mat = np.random.randint(low=-100, high=100, size=(n, n))
    mat = mat + mat.T
    row_sums = np.sum(np.abs(mat), axis=1)
...
def reconstruct_matrix(eigenvalues, eigenvectors):
    ...
    ans = np.matmul(np.matmul(eigenvectors, diag), eigenvectors.T)
...
def test():
    ...
    m = get_symmetric_invertible_matrix_3(n)
    print(f'My matrix:\n {m}\n')
